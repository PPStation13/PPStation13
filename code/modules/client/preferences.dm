...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	dat += "<a href='?_src_=prefs;preference=tab;tab=3' [current_tab == 3 ? "class='linkOn'" : ""]>Loadout</a>" // hippie -- Loadout system
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			dat += "<a href='?_src_=prefs;preference=all'>Soft Random: [soft_random ? "Yes" : "No"]</A><br>"
	...
	if(SSjob.occupations.len <= 0)
		...
		for(var/datum/job/job in SSjob.occupations)
			...
			if(is_banned_from(user.ckey, rank))
				...
				continue
			if(is_banned_from(user.ckey, list(CLUWNEBAN, CATBAN)) && rank != SSjob.overflow_role) // hippie start -- adds our jobban checks	
...
/datum/preferences/proc/process_link(mob/user, list/href_list)
	...
	process_hippie_link(user, href_list) // hippie -- process loadout stuff
	ShowChoices(user)
	return 1
